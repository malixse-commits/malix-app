(() => {
  ...
  function loadScript(src) {
    ...
    return new Promise((resolve, reject) => {
      const script = document.createElement('script'); script.src = src; script.dataset.malixModule = src;
      script.onload = () => { loaded.add(src); resolve(); }; script.onerror = () => reject(new Error(`Kunde inte ladda ${src}`));
  ...
  async function start() {
    ...
    for (const src of ['cleaning-tips.js','self-care.js','presence-care.js','presence-done.js','sleep-care.js','my-time-mobile.js','my-day-summary.js','inner-compass.js']) await loadScript(src);
  }
  start().catch(error => { console.error('Appens moduler kunde inte starta färdigt:', error); document.body.classList.add('calm-ready'); });
})();