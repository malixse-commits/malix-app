(()=>{
  ...
  let client=null,user=null,syncTimer=null,applying=false,reconciling=false;
  ...
  function applySnapshot(data){if(!data||typeof data!=='object')return;applying=true;try{Object.entries(data).forEach(([k,v])=>{if(isAppKey(k)){if(v===null||v===undefined)localStorage.removeItem(k);else localStorage.setItem(k,String(v))}})}finally{applying=false}document.dispatchEvent(new CustomEvent('malix-cloud-updated'))}
  ...
  function queuePush(){if(!user||applying)return;clearTimeout(syncTimer);syncTimer=setTimeout(()=>push().catch(e=>{console.error(e);setStatus('Kunde inte synka just nu','error')}),900)}
  ...
  async function signIn(email,password){const {data,error}=await client.auth.signInWithPassword({email,password});if(error)throw error;user=data.user;const source=await reconcile();renderAccount();setStatus(source==='remote'?'Senaste data från molnet är hämtad.':'Inloggad och synkad.','ok')}
  ...
  function renderAccount(){const p=accountPanel();if(!p)return;if(!configured()){p.innerHTML=`<p class="eyebrow">Konto & synkning</p><h3>☁️ Synkning förberedd</h3><p>Appen är redo för gemensam databas och inloggning, men databasen är ännu inte ansluten.</p><p data-cloud-status class="status"></p>`;return}
  ...
  function wire(){document.addEventListener('click',async e=>{const p=e.target.closest('#malixCloudPanel');if(!p)return;try{if(e.target.closest('[data-cloud-sync]')){setStatus('Synkar…');const source=await reconcile();setStatus(source==='remote'?'Senaste data från molnet är hämtad.':'Synkad.','ok');return}if(e.target.closest('[data-cloud-logout]')){await signOut();return}const email=p.querySelector('[data-cloud-email]')?.value.trim(),password=p.querySelector('[data-cloud-password]')?.value||'';if(e.target.closest('[data-cloud-login]')){if(!email||!password){setStatus('Fyll i e-post och lösenord.','error');return}setStatus('Loggar in…');await signIn(email,password);return}if(e.target.closest('[data-cloud-signup]')){if(!email||password.length<8){setStatus('Ange e-post och minst 8 tecken i lösenordet.','error');return}setStatus('Skapar konto…');await signUp(email,password)}}catch(err){console.error(err);setStatus(err?.message||'Något gick fel.','error')}});
    ...
    window.addEventListener('focus',()=>{if(user)reconcile().then(source=>{if(source==='remote')setStatus('Senaste data från molnet är hämtad.','ok')}).catch(console.error)});
    document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&user)reconcile().then(source=>{if(source==='remote')setStatus('Senaste data från molnet är hämtad.','ok')}).catch(console.error)});
  }

  async function init(){renderAccount();wire();if(!configured())return;client=window.supabase.createClient(CONFIG.url,CONFIG.anonKey,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});const {data}=await client.auth.getSession();user=data.session?.user||null;renderAccount();if(user){try{const source=await reconcile();if(source==='remote')setStatus('Senaste data från molnet är hämtad.','ok')}catch(e){console.error(e);setStatus('Kunde inte läsa molndata just nu.','error')}}client.auth.onAuthStateChange((_event,session)=>{user=session?.user||null;renderAccount()})}
