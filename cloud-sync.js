(()=>{
  ...
  let client=null,user=null,syncTimer=null,applying=false,lastRemoteAt='';
  ...
  async function reconcile(){const remote=await remoteRow(),m=meta(),local=snapshot();if(!remote){await push();return 'local'}
    const remoteAt=remote.updated_at||'',localAt=m.lastLocalChangeAt||'';
    if(remoteAt&&remoteAt>localAt){applySnapshot(remote.state);lastRemoteAt=remoteAt;saveMeta({...m,lastSyncAt:remoteAt,userId:user.id});return 'remote'}
  ...
  function wire(){document.addEventListener('click',async e=>{const p=e.target.closest('#malixCloudPanel');if(!p)return;try{if(e.target.closest('[data-cloud-sync]')){setStatus('Synkar…');await reconcile();setStatus('Synkad.','ok');return}if(e.target.closest('[data-cloud-logout]')){await signOut();return}const email=p.querySelector('[data-cloud-email]')?.value.trim(),password=p.querySelector('[data-cloud-password]')?.value||'';if(e.target.closest('[data-cloud-login]')){if(!email||!password){setStatus('Fyll i e-post och lösenord.','error');return}setStatus('Loggar in…');await signIn(email,password);return}if(e.target.closest('[data-cloud-signup]')){if(!email||password.length<8){setStatus('Ange e-post och minst 8 tecken i lösenordet.','error');return}setStatus('Skapar konto…');await signUp(email,password)}}catch(err){console.error(err);setStatus(err?.message||'Något gick fel.','error')}});
    const origSet=Storage.prototype.setItem,origRemove=Storage.prototype.removeItem;
    Storage.prototype.setItem=function(k,v){const r=origSet.call(this,k,v);if(this===localStorage&&isAppKey(k)&&!applying){const m=meta();saveMeta({...m,lastLocalChangeAt:new Date().toISOString()});queuePush()}return r};
    Storage.prototype.removeItem=function(k){const r=origRemove.call(this,k);if(this===localStorage&&isAppKey(k)&&!applying){const m=meta();saveMeta({...m,lastLocalChangeAt:new Date().toISOString()});queuePush()}return r};
    window.addEventListener('focus',()=>{if(user)reconcile().then(source=>{if(source==='remote')location.reload()}).catch(console.error)});
    document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&user)reconcile().then(source=>{if(source==='remote')location.reload()}).catch(console.error)});
  }
