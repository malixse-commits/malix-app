(() => {
 ...
 function render(){
  ...
  const cards=groups[selected].map(x=>`<article class="recipe-card"><p class="eyebrow">${new Date(x.date).toLocaleDateString('sv-SE')}</p><h3>${esc(x.type)}</h3><p>${esc(x.text)}</p>${x.extra?`<p class="note">${esc(x.extra)}</p>`:''}</article>`).join('');
 ...
 document.addEventListener('click',e=>{if(e.target.closest('[data-care="careReflection"]'))setTimeout(render,0)},true);
