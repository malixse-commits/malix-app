(() => {
  ...
  function add(kind,label,emoji){
    ...
    let selected='';
    box.addEventListener('click',e=>{const b=e.target.closest('[data-choice]');if(b){selected=b.dataset.choice;box.querySelectorAll('[data-choice]').forEach(x=>x.classList.toggle('active',x===b));return}if(e.target.closest('[data-save]')){if(!selected){alert(`Välj ${kind==='movement'?'en rörelse':'en återhämtning'} först.`);return}const data=load(),d=today();data[d]=data[d]||{movement:[],recovery:[]};data[d][kind].push({type:selected,minutes:Number(box.querySelector('[data-minutes]').value),feeling:box.querySelector('[data-feeling]').value});save(data);render(box,kind);selected='';box.querySelectorAll('[data-choice]').forEach(x=>x.classList.remove('active'));}});
    render(box,kind);
  }
  function render(box,kind){const data=load()[today()]?.[kind]||[];const out=box.querySelector('[data-summary]');if(!data.length){out.textContent='Inget sparat idag – och det är helt okej.';return}const total=data.reduce((s,x)=>s+x.minutes,0);out.innerHTML=`<strong>Idag:</strong> ${data.map(x=>`${x.type} ${x.minutes}${x.minutes>=60?'+':''} min`).join(' · ')}<br><strong>Totalt:</strong> ${total} min`;}
