(() => {
  ...
  const aliases={
    'ostskiva':['ost'], 'brodskiva':['brod'], 'rostat brod':['brod'], 'knackebrod':['knackebrod','brod'],
  ...
  function formatAmount(a){
    if(a.u==='ml'&&a.n>=1000)return `${Math.round(a.n/10)/100} l`;
    if(a.u==='g'&&a.n>=1000)return `${Math.round(a.n/10)/100} kg`;
    return `${Math.max(0,Math.round(a.n*100)/100)} ${a.u}`.trim();
  }
  ...
  function matches(stockName,food){
    const sn=norm(stockName),fn=norm(food),candidates=aliases[fn]||[fn];
    return candidates.some(a=>{const an=norm(a);return sn===an||sn.includes(an)||an.includes(sn)});
  }
  ...
  function deduct(items){
    const st=load();if(!st.stock.length||!items.length)return {changed:false,items:[]};
    let changed=false;const results=[];
    items.forEach(({food,quantity})=>{
      const stock=st.stock.find(x=>matches(x.item,food));
      if(!stock){results.push(`${food}: finns inte i Mitt kök`);return}
      const have=parseAmount(stock.amount),raw=parseAmount(quantity);
      if(!have||!raw){results.push(`${food}: mängden kunde inte räknas`);return}
      ...
      results.push(`${food}: ${formatAmount(used)} använt, ${stock.amount} kvar`);
      if(remain<=0){
