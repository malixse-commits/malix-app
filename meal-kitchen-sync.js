(() => {
  ...
  function deduct(items){
    ...
    items.forEach(({food,quantity})=>{
      ...
      const have=parseAmount(stock.amount),used=parseAmount(quantity); if(!have||!used)return;
      if(have.u!==used.u){
        // Common household conversions: pieces stay pieces; dl/ml normalize above.
        return;
      }
  ...
  document.addEventListener('submit',event=>{
    ...
    if(submit&&/^Spara ändringar/i.test(submit.textContent||'')) return; // undvik dubbel avräkning vid redigering
