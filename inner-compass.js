(()=>{
 ...
 function renderShiftPrompt(){
   ...
   box.hidden=false;
   box.innerHTML=`<p class="eyebrow">🌿 Lägg märke till en möjlig förflyttning</p><h3>Verkar något ha förändrats mot slutet?</h3><p>Började tankarna gå från ${shortCircle(s.from)} mot att ${target}? Det <strong>kan</strong> vara ${shortCircle(s.to)} som aktiveras. Du avgör själv om det stämmer.</p><label><strong>Vad märkte du när förflyttningen kom?</strong><span class="note">Lägg märke till kroppen, tankarna, känslorna eller vad du fick lust att göra.</span><textarea id="compassShiftBody" rows="2" placeholder="Skriv om du vill"></textarea></label><div class="chips"><button type="button" class="primary" data-shift-answer="yes">Ja, jag känner igen det</button><button type="button" class="secondary" data-shift-answer="no">Nej, jag är fortfarande kvar här</button><button type="button" class="secondary" data-shift-answer="unsure">Jag vet inte</button></div><p class="note">Inget system är bättre än något annat. Poängen är att bli nyfiken på hur du rör dig mellan dem.</p>`;
 }
 function transitionHtml(x){
   ...
   return `<div class="note" style="margin-top:10px"><strong>🌿 Möjlig förflyttning:</strong> ${shortCircle(s.from)} → ${shortCircle(s.to)}<br>${safe(answer)}${s.body?`<br><strong>Vad märkte du när förflyttningen kom?</strong> ${safe(s.body)}`:''}</div>`
 }
