(() => {
  ...
  function setup(){
    ...
    box.addEventListener('click',e=>{
      const b=e.target.closest('[data-takeaway]');
      if(b){selected=b.dataset.takeaway;box.querySelectorAll('[data-takeaway]').forEach(x=>x.classList.toggle('active',x===b));return}
      if(!e.target.closest('[data-use-takeaway]'))return;
      if(!selected){alert('Välj typ av hämtmat först.');return}
      const detail=box.querySelector('[data-takeaway-detail]').value.trim();
      const amount=box.querySelector('[data-takeaway-amount]').value.trim()||'1 portion';
      const food=form.querySelector('#foodText,[name="food"],textarea');
      if(!food)return;
      food.value=`Hämtmat: ${detail||selected} (${amount})`;
      food.dataset.takeaway='1';
      food.dispatchEvent(new Event('input',{bubbles:true}));

      // Gå vidare till appens vanliga sparläge i stället för att stanna i hämtmatsrutan.
      form.dataset.simpleStep='other';
      const foodLabel=food.closest('label');
      if(foodLabel)foodLabel.hidden=false;
      const status=document.querySelector('#mealSaved');
      if(status)status.textContent='Hämtmaten är ifylld. Spara måltiden nedan.';
      const saveButton=form.querySelector('button[type="submit"]');
      setTimeout(()=>saveButton?.scrollIntoView({behavior:'smooth',block:'center'}),0);
    });
